...
void ScenicManager(mgraphtype *g);                              /*景点管理*/
void Manager();                                                 /*管理员界面*/
int Login();                                                    /*登录系统*/
int Login_System();                                             /*登录首界面*/
void Information_print();                                       /*公告栏*/
void main();                                                    /*主函数*/
...
void Create_Map(mgraphtype *g)
{
	int i, j, k, e;
	FILE *rf;
	rf = fopen("map.txt", "r");                               /*读取文件*/
	if (rf)
	{
		fscanf(rf, "%d%d", &g->vexNum, &g->edgNum);	          /*读取顶点数和边数*/
		for (i = 0; i < g->vexNum; i++)
			fscanf(rf, "%d%s%s", &g->vexs[i].num, g->vexs[i].name, g->vexs[i].intro);/*读取景点编号、名称、介绍*/
		for (i = 0; i < g->vexNum; i++)
			for (j = 0; j < g->vexNum; j++)
			{
				if (i == j)                                  /*自环的距离为零*/
					...
					g->edge[i][j] = INFINITY;                /*不存在路径距离无穷*/
			}
		for (k = 0; k < g->edgNum; k++)
		{
			fscanf(rf, "%d%d%d", &i, &j, &e);                /*建立邻接矩阵*/
...
void Search_Location(mgraphtype *g) {
	...
	printf("\n景点名称：[%s]\n\n", g->vexs[s - 1].name);          /*打印景点名称*/
	printf("景点简介： %s\n\n", g->vexs[s - 1].intro);            /*打印景点信息*/
}
```
```c
		...
		for (i = 0; i < g->vexNum; i++)
			for (j = 0; j < g->vexNum; j++)                        
				if (dist[i][j] >(dist[i][k] + dist[k][j])) {
					dist[i][j] = dist[i][k] + dist[k][j];          /*更新dist[i][j]的值*/
...
void main() {
	...
	if (Login_System()) {
		while (!quit) {                                                          /*系统退出条件满足判定*/
			...
			case 4:system("cls"); Dis_Map(); BestPath(&g); break;                /*多景点间最佳访问路线查询*/
